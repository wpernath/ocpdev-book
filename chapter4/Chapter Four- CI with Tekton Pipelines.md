...
For our well known [person-service][3], we are going to create a Tekton Pipeline, which shouldn’t do much more than compiling the source, creating a Docker image based on [JIB][4], pushing it to [Quay.io][5] and using [kustomize][6] to apply that image to an OpenShift project called book-tekton. 
...
3. **`quarkus.container-image.builder`**: Selects the method of building the container image. We want to use [JIB][7].
4. **`quarkus.container-image.image`**: The complete name of the image to be build. 

Now checkout the [source code][8], have a look at `person-service/src/main/resources/application.properties`, change the image property to your needs and call
...
- For image generation and pushing by executing `package` and providing the following parameters to [instruct quarkus to build and push the image][10]
...
As we want to use Kustomize to apply the new image to our Deployment, we have to look for a proper task in the [Tekton Hub][11]. Unfortunately, there doesn’t seem to be one available, so we have to create our own one.

For this, we first need to have a proper image which contains the kustomize executable. The Dockerfile for this is available [here][12] and the image is available [here][13].
